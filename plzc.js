...
function setSjsj ( row, col, value ) {
	...
	if ( getTableValue( row, 12 ).indexOf( "cell" ) != -1 || getTableValue( row, 12 ).indexOf( "---" ) != -1 || getTableValue( row, 12 ).indexOf( "***" ) != -1 ) {
		...
		// console.log(formatDateText("D", sjsj));
	}
...
title.textContent = "计量日冻结数据批量召测神器";
...
TOPpage.appendChild( YHBHS_textarea );
...
TOPpage.appendChild( initTableBtn );


function fetch_0_get_danwei_name(){//预留作为其他地市局单位使用，@@@FLAG@@@
...
gotoShowSaveButton.onclick = function () {
	if (initTableFlag == 0) {
		alert("请先初始化表格")
		return
	}
...
TOPpage.appendChild( gotoShowSaveButton );

// Create a button on the right side of the textarea
var abortTestButton = document.createElement( "button" );
abortTestButton.innerText = "终止所有召测并解锁按钮";
...
TOPpage.appendChild( abortTestButton );
...
TOPpage.appendChild( checkKEYButton );
...
TOPpage.appendChild( savaAllButton );
...
TOPpage.appendChild( saveToExcelButton );
...
var totalF = 0;
		// Add some content to the page
		var tipsF3 = document.createElement("p");
...
tableDiv.style.height = DIVheight - 280 + "px"; //Change this to adjust the height of the page
...
function fetch_1_zdljdz ( yhbh, row1, col1, row2, col2, continueFlag ) { //continueFlag接力赛标志
	...
	fetch( "http://10.150.12.240/gdcams/treeSearchMatch.html?method=viewMatchDyyhList", {
			"headers": {
				"accept": "application/json, text/javascript, */*; q=0.01",
			...
			"referrer": "http://10.150.12.240/gdcams/treeSearchMatch.html?method=viewMatchDyyh&treeFlag=2&json=",
			"referrerPolicy": "strict-origin-when-cross-origin",
			"body": "isSearch=true&yhbh=" + yhbh,
			"method": "POST",
			"mode": "cors",
			"credentials": "include",
			signal: controllerww.signal
...
function fetch_2_cbsxh ( zdbs, yhbh, row1, col1, row2, col2, continueFlag ) {
	...
	fetch( "http://10.150.12.240/gdcams/baseInfPRMSMp.html?method=getPRMSMPList&zdbs=" + zdbs + "&cjqbs=", {
			"headers": {
				"accept": "application/json, text/javascript, */*; q=0.01",
				"accept-language": "zh-CN,zh;q=0.9,en-US;q=0.8,en;q=0.7",
				"content-type": "application/x-www-form-urlencoded; charset=UTF-8",
			...
			"referrer": "http://10.150.12.240/gdcams//baseInfPRMSMp.html?method=view",
			"referrerPolicy": "strict-origin-when-cross-origin",
			"body": "yhbh=" + yhbh,
			"method": "POST",
			"mode": "cors",
		...
		.then( ( responseJson ) => {
			...
			if(responseJson.total==1){
				...
				// console.log("dycldbs and cbsxh was writed");
...
//3，召测参数
function fetch_3_call_for_testing ( dycldbs, qxdwbm, zdljdz, zdbs, cbsxh, row, startCol, continueFlag ) {
	// if(syn_lock==1){
		setTableButtonHide( row, 20 );
		setTableButtonHide( row, 28 );
	setTableValuePlus( row, startCol, [ "***", "***", "***", "***", "***", "***" ] );
	setTableButtonColor( row, 5, "orange" );
	fetch( "http://10.150.12.240/gdcams/baseInfPRMSMp.html?method=_callBackZj", {
			"headers": {
				"accept": "application/json, text/javascript, */*; q=0.01",
				"accept-language": "en-US,en;q=0.9",
				"content-type": "application/x-www-form-urlencoded; charset=UTF-8",
			...
			"referrer": "http://10.150.12.240/gdcams/baseInfPRMSMp.html?method=viewRdjzjzc&dycldbs=" + dycldbs,
			"referrerPolicy": "strict-origin-when-cross-origin",
			"body": "dycldbs=" + dycldbs + "&qxdwbm=" + qxdwbm + "&zdljdz=" + zdljdz + "&zdbs=" + zdbs + "&cbsxh=" + cbsxh,
			"method": "POST",
			"mode": "cors",
			"credentials": "include",
			signal: controllerww.signal
...
//4，召测时间
function fetch_4_call_for_time ( dycldbs, qxdwbm, zdljdz, zdbs, cbsxh, dbh, sbwdk, btl, txfsdm, sjw, tzw, row, col, continueFlag ) {
	setTableButtonHide( row, 20 );
	setTableButtonHide( row, 28 );
	setTableValue( row, col, "***" );
	setTableButtonColor( row, 11, "orange" );
	fetch( "http://10.150.12.240/gdcams/baseInfPRMSMp.html?method=_callBackSj", {
			"headers": {
				"accept": "application/json, text/javascript, */*; q=0.01",
				"accept-language": "en-US,en;q=0.9",
				"content-type": "application/x-www-form-urlencoded; charset=UTF-8",
			...
			"referrer": "http://10.150.12.240/gdcams/baseInfPRMSMp.html?method=viewRdjzjzc&dycldbs=" + dycldbs,
			"referrerPolicy": "strict-origin-when-cross-origin",
			"body": "dycldbs=" + dycldbs + "&qxdwbm=" + qxdwbm + "&zdljdz=" + zdljdz + "&zdbs=" + zdbs + "&cbsxh=" + cbsxh + "&dbh=" + dbh + "&sbwdk=" + sbwdk + "&btl=" + btl + "&txfsdm=" + txfsdm + "&sjw=" + sjw + "&tzw=" + tzw,
			"method": "POST",
			"mode": "cors",
			"credentials": "include",
			signal: controllerww.signal
		...
		.then( ( responseJson ) => {
			...
			if ( responseJson.message == "处理成功" ) {
				...
				if ( continueFlag ) {
					...
					// fetch_5_call_for_data( dycldbs, qxdwbm, zdljdz, zdbs, cbsxh, dbh, sbwdk, btl, txfsdm, sjw, tzw, sjxitem, sjxitem2, row, 15, continueFlag );
				}
...
//召读数据
function fetch_5_call_for_data ( dycldbs, qxdwbm, zdljdz, zdbs, cbsxh, dbh, sbwdk, btl, txfsdm, sjw, tzw, sjxitem, sjxitem2, row, startCol, continueFlag ) {
	if(startCol==15){
		setTableButtonHide( row, 20 );
	}
	if(startCol==23){
	...
	fetch( "http://10.150.12.240/gdcams/baseInfPRMSMp.html?method=_callBackCs", {
			"headers": {
				"accept": "application/json, text/javascript, */*; q=0.01",
				"accept-language": "en-US,en;q=0.9",
				"content-type": "application/x-www-form-urlencoded; charset=UTF-8",
			...
			"referrer": "http://10.150.12.240/gdcams/baseInfPRMSMp.html?method=viewRdjzjzc&dycldbs=" + dycldbs,
			"referrerPolicy": "strict-origin-when-cross-origin",
			"body": "dycldbs=" + dycldbs + "&qxdwbm=" + qxdwbm + "&zdljdz=" + zdljdz + "&zdbs=" + zdbs + "&cbsxh=" + cbsxh + "&dbh=" + dbh + "&sbwdk=" + sbwdk + "&btl=" + btl + "&txfsdm=" + txfsdm + "&sjw=" + sjw + "&tzw=" + tzw + "&sjxitem=" + sjxitem + "&sjxitem2=" + sjxitem2,
			"method": "POST",
			"mode": "cors",
			"credentials": "include"
		} )
		...
		.then( ( responseJson ) => {
			if ( responseJson.message == "处理成功" ) {
				...
				// 	}
				// 	var [ dycldbs, qxdwbm, zdljdz, zdbs, cbsxh, dbh, sbwdk, btl, txfsdm, sjw, tzw ] = list;
				// 	var sjxitem = document.querySelector( "#cell" + row + "_" + 21 + " > select" )
				// 		.sjxitem;
				// 	var sjxitem2 = document.querySelector( "#cell" + row + "_" + 22 + " > p:nth-child(1)" )
				// 		.innerText;
					
				// 		// getTableButton(row,22).click()
				// 	// fetch_5_call_for_data( dycldbs, qxdwbm, zdljdz, zdbs, cbsxh, dbh, sbwdk, btl, txfsdm, sjw, tzw, sjxitem, sjxitem2, row, 23, false );
				// }
...
function fetch_6_savedata ( dycldbs, qxdwbm, zdljdz, zdbs, dg, dp, df, dj, dz, sjsj, sjxitem, sjxitem2, row, col ) {
	...
	fetch( "http://10.150.12.240/gdcams/baseInfPRMSMp.html?method=saveZj", {
		"headers": {
			"accept": "application/json, text/javascript, */*; q=0.01",
			"accept-language": "en-US,en;q=0.9",
			"content-type": "application/x-www-form-urlencoded; charset=UTF-8",
		...
		"referrer": "http://10.150.12.240/gdcams/baseInfPRMSMp.html?method=viewRdjzjzc&dycldbs=" + dycldbs,
		"referrerPolicy": "strict-origin-when-cross-origin",
		"body": "dycldbs=" + dycldbs + "&qxdwbm=" + qxdwbm + "&zdljdz=" + zdljdz + "&zdbs=" + zdbs + "&dg=" + dg + "&dp=" + dp + "&df=" + df + "&dj=" + dj + "&dz=" + dz + "&sjsj=" + sjsj + "&sjxitem=" + sjxitem + "&sjxitem2=" + sjxitem2,
		"method": "POST",
		"mode": "cors",
		"credentials": "include"
	} )
	...
	.then( ( responseJson ) => {
		if ( responseJson.result == "success" ) {
			...
			flashSum()
			// document.querySelector( "#cell" + row + "_" + col + " > button" ).style.color = "green"; //让保存数据按钮变绿
		}
	} );
}
...
// 定义一个函数，接受一个日期对象作为参数
function getHashKEY(date,name) {
    // 获取日期对象的年份和月份
    let year = date.getFullYear();
    let month = date.getMonth() + 1; // 月份从0开始，所以要加1
    let str = name +"$"+ year + "+"+month+"@F1";

	return kbt.encrypt(str)
...
function safeSaveFetch(){
    ...
    fetch("http://10.150.12.240/gdcams/staggerSituation.html?method=getTxCount", {
  "headers": {
    "accept": "application/json, text/javascript, */*; q=0.01",
    "accept-language": "zh-CN,zh;q=0.9,en-US;q=0.8,en;q=0.7",
    "x-requested-with": "XMLHttpRequest"
  },
  "referrer": "http://10.150.12.240/gdcams/index.html",
  "referrerPolicy": "strict-origin-when-cross-origin",
  "body": null,
  "method": "POST",
  "mode": "cors",
  "credentials": "include"
    ...
    .then( ( responseJson ) => {
        // console.log(responseJson);
        if ( responseJson.d.indexOf("年")!=-1 ) {
            var now=responseJson.d;
            now=now.replace("年","-");
            now=now.replace("月","-");
            now=now.replace("日","");
		...
		var name=document.querySelector("#leftTree > div.left_box1 > div.left_box_title > table > tbody > tr > td:nth-child(2) > ul > li:nth-child(1) > span").innerText;
		// console.log(getHashKEY(nowDate,name));
		var gdsname=document.querySelector("#leftTree > div.left_box1 > div.left_box_title > table > tbody > tr > td:nth-child(2) > ul > li:nth-child(2)").innerText;
		//个人码，单位码，游客码，管理员码
		//getHashKEY(new Date(),"黄辅英")
		var decryptStr=kbt.decrypt(KEY_input.value)

		if(decryptStr==kbt.decrypt(getHashKEY(nowDate,name))||decryptStr==kbt.decrypt(getHashKEY(nowDate,gdsname))||decryptStr==kbt.decrypt(getHashKEY(nowDate,"GUEST"))||decryptStr==kbt.decrypt(getHashKEY(new Date("1998-1-31"),"ADMIN"))){
			activeLOCK=1;
			tips2a.textContent="已激活"
			tips2a.style.color="green"
		}else{
			activeLOCK=0;
			tips2a.textContent="激活码不正确"
			tips2a.style.color="red"
		}
		if(name=="黄辅英"){
			activeLOCK=1;
			tips2a.textContent="已激活"
